(() => {
  ...
  const translate = value => {
    ...
    if (normalized === 'Priority stop · Added from your request') return '优先地点 · 根据你的要求添加';
    ...
    if (normalized === 'DAY ROUTE') return '每日路线';
    if (normalized === 'See your day at a glance') return '一眼查看当天行程';
    if (normalized === 'Open map') return '打开地图';
    if (normalized === 'Loading your lightweight map…') return '正在加载轻量地图…';
    if (normalized === 'City preview') return '城市预览';
    if (normalized === 'Map unavailable') return '地图暂不可用';
    if (normalized === 'Map could not load. Check your connection and try again.') return '地图加载失败，请检查网络后重试。';
    if (normalized === 'Food and experiences you logged today') return '今天记录的餐饮和体验花费';
    if (normalized === 'Edit this plan') return '编辑这项行程';
    if (normalized === 'EDIT TIMELINE ITEM') return '编辑时间线项目';
    if (normalized === 'Make it yours') return '编辑你的行程';
    if (normalized === 'Editing as') return '编辑人';
    if (normalized === 'Actual cost') return '实际花费';
    if (normalized === 'Save changes') return '保存修改';
    if (normalized === 'Remove from timeline') return '从时间线移除';
    if (normalized === 'Timeline order updated') return '时间线顺序已更新';
    if (normalized === 'Item removed from timeline') return '已从时间线移除项目';
    if (normalized === 'LOCAL MOODBOARD') return '本地灵感册';
    if (normalized === 'Landscape') return '风景';
    if (normalized === 'Culture') return '文化';
    if (normalized === 'Taste') return '美食';
    if (normalized === '✦ Explore') return '✦ 探索';
    if (placeNames[value]) return placeNames[value];
    let match = String(value).match(/^(\d+) Days in (.+)$/); if (match) return `${match[2]} · ${match[1]} 天`;
    match = String(value).match(/^DAY\s+(\d+)\s+·\s+(SUN|MON|TUE|WED|THU|FRI|SAT),\s+(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)\s+(\d+)$/i); if (match) { const weeks={SUN:'周日',MON:'周一',TUE:'周二',WED:'周三',THU:'周四',FRI:'周五',SAT:'周六'}; const months={JAN:'1月',FEB:'2月',MAR:'3月',APR:'4月',MAY:'5月',JUN:'6月',JUL:'7月',AUG:'8月',SEP:'9月',OCT:'10月',NOV:'11月',DEC:'12月'}; return `第${Number(match[1])}天 · ${months[match[3].toUpperCase()]}${match[4]}日（${weeks[match[2].toUpperCase()]}）`; }
    ...
    match = String(value).match(/^(\d+) mapped stops$/i); if (match) return `已标记 ${match[1]} 个地点`;
    match = String(value).match(/^(.+) in little moments$/i); if (match) return `${localizeCity(match[1])} 的城市瞬间`;
    match = String(value).match(/^Heads up: these dates are in a popular travel period\. Flights and hotels may be more expensive, so booking early is a good idea\.$/i); if (match) return '提醒：这些日期属于热门旅行时段，机票和酒店可能更贵，建议尽早预订。';
    ...
    match = String(value).match(/^Timeline item updated by (.+)$/i); if (match) return `时间线项目已由 ${match[1]} 更新`;
    match = String(value).match(/^Edited by (.+)$/i); if (match) return `编辑人：${match[1]}`;
