(() => {
  ...
  const translate = value => {
    ...
    match = String(value).match(/^(.+) · A dining choice near your stay$/); if (match) return `${match[1]} · 住宿附近的餐饮选择`;
    match = String(value).match(/^(.+) · Popular local highlight$/); if (match) return `${match[1]} · 当地热门景点`;
    match = String(value).match(/^(.+) · Chosen near your stay$/); if (match) return `${match[1]} · 根据住宿位置挑选`;
