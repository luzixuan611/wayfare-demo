(() => {
  ...
  extra.innerHTML = `<label>Return date<input id="welcome-return-date" type="date" value="2026-06-16" /></label><label>Departure city <small>Used to prepare your flight search</small><input id="welcome-origin" placeholder="e.g. Berlin, Germany" /></label><div class="preference-row"><label>Travelers<input id="welcome-travelers" type="number" min="1" max="12" value="2" /></label><label>Booking budget / person<input id="welcome-budget" type="number" min="0" value="4000" placeholder="e.g. 4000" /></label></div><label>On-trip budget / person <small>Food, activities, and small daily expenses</small><input id="welcome-spending-budget" type="number" min="0" value="600" placeholder="e.g. 600" /></label><label>Currency<select id="welcome-currency"><option value="¥">RMB — Chinese yuan</option><option value="$">USD — US dollar</option></select></label><label>Where would you like to stay?</label><div class="stay-options"><button type="button" class="stay-option selected" data-stay="hotel"><span>⌂</span><b>Hotel</b></button><button type="button" class="stay-option" data-stay="homestay"><span>⌘</span><b>Homestay</b></button><button type="button" class="stay-option" data-stay="hostel"><span>◌</span><b>Hostel</b></button></div>`;
  ...
  window.wayfarePreferences = () => ({
    ...
    spendingBudget: Number(document.querySelector('#welcome-spending-budget').value || 0),
    returnDate: document.querySelector('#welcome-return-date').value,
    origin: document.querySelector('#welcome-origin').value.trim(),
    currency: document.querySelector('#welcome-currency').value,
    accommodation
  });
})();
