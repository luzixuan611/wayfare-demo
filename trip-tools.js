(() => {
  ...
  window.wayfareVotes = { isWinner(dayIndex, entry) { const votes=get('votes'); const restaurantVotes=Object.entries(votes).filter(([id])=>id.startsWith('restaurant-')); const max=Math.max(...restaurantVotes.map(([,list])=>list.length),0); return max>0 && restaurantVotes.some(([id,list])=>list.length===max && String(entry[3]).includes(id.slice('restaurant-'.length))); } };
  function renderVotes() {
    const votes = get('votes'); const candidates = (api.recommendations || []).filter(item => item[0] === '🍜' || item[0] === '☕').map(item => ({ id:`restaurant-${item[1]}`, title:item[1] })); const custom = get('custom-picks').items || [];
    ...
    drawer.innerHTML = `<div class="tool-heading"><button class="tool-back" type="button">←</button><div><b>Where should we eat?</b><small>Vote on restaurant choices near your stay</small></div></div><label class="tool-label">Voting as<select id="vote-person">${people().map(person => `<option value="${escape(person.name)}">${escape(person.avatar)} ${escape(person.name)}</option>`).join('')}</select></label><div class="vote-list">${candidates.length ? candidates.map(row).join('') : '<p class="tool-empty">Restaurant choices will appear after the itinerary is generated.</p>'}${custom.length ? `<p class="custom-picks-label">Custom picks</p>${custom.map(item => row({id:item.id,title:item.title})).join('')}` : ''}</div><form class="custom-vote" id="custom-vote-form"><label>Add a custom vote<input name="title" required placeholder="e.g. Sunset cruise or cooking class" /></label><button type="submit">＋ Add option</button></form><small class="tool-note">Demo votes are saved in this browser.</small>`;
    drawer.querySelector('.tool-back').onclick = close;
    drawer.querySelectorAll('[data-vote]').forEach(button => button.onclick = () => { const voter = drawer.querySelector('#vote-person').value; const id = button.dataset.vote; const next = get('votes'); const list = next[id] || []; next[id] = list.includes(voter) ? list.filter(name => name !== voter) : [...list, voter]; save('votes', next); api.render(); renderVotes(); });
