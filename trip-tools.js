(() => {
  ...
  window.wayfareVotes = { isWinner(dayIndex, entry) { const votes=get('votes'); const prefix=`${dayIndex}-${entry[0]}-${entry[3]}`; const count=(votes[prefix]||[]).length; const dayCounts=Object.entries(votes).filter(([id])=>id.startsWith(`${dayIndex}-`) && !id.startsWith(`${dayIndex}-custom-`)).map(([,list])=>list.length); return count>0 && count===Math.max(...dayCounts,0); } };
